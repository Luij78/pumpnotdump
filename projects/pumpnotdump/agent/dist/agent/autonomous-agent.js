...
class AntiRugAgent {
    constructor() {
        ...
        // Load agent wallet
        const keypairPath = path_1.default.join(process.env.HOME, ".config/solana/skipper-wallet.json");
        const keypairData = JSON.parse(fs_1.default.readFileSync(keypairPath, "utf-8"));
        this.agentWallet = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(keypairData));
        // Initialize Anchor provider and program
        const provider = new anchor.AnchorProvider(this.connection, new anchor.Wallet(this.agentWallet), { commitment: "confirmed" });
        anchor.setProvider(provider);
        // Load program IDL
        const idlPath = path_1.default.join(__dirname, "../pumpnotdump/target/idl/pumpnotdump.json");
     ...
     */
    async calculateRugScore(launch) {
        ...
        try {
            ...
            const [rugScorePDA] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("rug_score"), launch.account.tokenMint.toBuffer()], this.program.programId);
...
Analyzed by pump.notdump.fun Anti-Rug Agent #${AGENT_ID}`;
     ...
     */
    async postStatusUpdate(rugScore) {
...
exports.AntiRugAgent = AntiRugAgent;
// Main execution
async function main() {
